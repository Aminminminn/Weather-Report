...
public class CarouselEntry {
	 ...
	 * @param image            the image to draw on top
	 * @param temperatureLabel the entry string
	 */
	public CarouselEntry(int id, Image image, String temperatureLabel, String hourLabel) {
		this.id = id;
	 ...
	 * @param contentWidth  the content width of the carousel
	 * @param contentHeight the content height of the carousel
	 * @param font          the font to render with.
	 * @param stopped       whether the carousel is currently stopped
	 * @param sizeRatio     the size ratio
	 * @param offsetX       the offset on the X-axis
	 * @param offsetY       the offset on the Y-axis
	 * @param isDnd         whether the entry is being dragged
	 */
	public void render(GraphicsContext g, int contentWidth, int contentHeight, Font font, boolean stopped, //NOSONAR Every parameter is useful.
		...
		if (sizeRatio >= DRAW_STRING_RATIO && !isDnd) {
			...
			StringPainter.drawStringInArea(g, this.temperatureLabel, font, stringX, stringY - 30, stringWidth,
					stringHeight, Alignment.HCENTER, Alignment.TOP);
			StringPainter.drawStringInArea(g, this.hourLabel, Font.getDefaultFont(), stringX, stringY, stringWidth,
					stringHeight, Alignment.HCENTER, Alignment.TOP);
		}
	}
