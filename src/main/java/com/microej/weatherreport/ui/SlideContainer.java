 ...
 * <p>
 * Each child is the size of the content of the container. Hence, only the last
 * added child is visible.
 * <p>
 * When adding a child, an animation is done by translating the departing child
 * and the new one from the right of the screen. When removing a child, an
 * animation is done by translating the departing child and the new one to the
 * right of the screen.
 * <p>
...
public class SlideContainer extends Container {
	 ...
	 * {@inheritDoc}
	 * <p>
	 * An animation is started to slide the widget from outside the container on the
	 * right to its final position.
	 */
	@Override
	 ...
	 * <p>
	 * An animation is started to slide the widget from its current position to the
	 * outside of the container on the right.
	 */
	public void removeLast() {
		...
		if (childrenCount > 1) {
			...
			hideChild(newlyVisibleChild, child);
		}
	}

	/**
	 * Starts the animation translating the child from the given position to its
	 * final position (<code>0</code>).
	 *
	 * @param child  the child to move.
	 * @param startX the initial position.
	 */
	...
	/**
	 * Starts the animation translating the child from the given position to outside
	 * the container (content width).
	 *
	 * @param child the child to move.
	 */
	private void hideChild(Widget newlyVisibleChild, Widget child) {
		removeChild(child);
		this.previousPosition = 0;
		doAnimation(newlyVisibleChild, child, 0, getContentWidth());
	}
	...
	private void doAnimation(final Widget leftChild, final Widget rightChild, int startX, int endX) {
		...
		long duration = (long) TRANSITION_DURATION * Math.abs(endX - startX) / getContentWidth();
		Motion motion = new Motion(QuadEaseOutFunction.INSTANCE, startX, endX, duration);
		this.releasedAnimation = new MotionAnimation(getAnimator(), motion, (value, finished) -> {
			if (finished) {
				restore();
			} else {
	...
	public void render(GraphicsContext g) {
		if (this.moving) {
			...
			if (shift > 0) {
				...
				// Draw the part of the left widget previously outside the container.
				g.setClip(contentX, contentY, shift, contentHeight);
			} else {
				// The widgets are moved to the left.
				// Draw the part of the right widget previously outside the container.
	...
	private boolean onPointerDragged(int contentWidth, int childrenCount, int pointerX, int pointerY) {
		...
		if (this.pressed && shiftX != 0) {
			...
			if (!this.moving && Math.abs(shiftX) > Math.abs(shiftY) && shiftX > 0) {
				...
				Animation draggedAnimation = platformTimeMillis -> {
	...
	private boolean onPointerReleased(final int contentWidth, final int childrenCount) {
		if (this.moving) {
			MicroUI.callSerially(() -> {
				...
				// Depending on the position of the child with the middle of the container, keep
				// it or remove it.
